...
public class MainActivity extends Activity implements ReturnFromBackgroundTask {
	...
	private void initilizeOnClickListener() {
		...
		buttonSetWallpaper.setVisibility(Button.INVISIBLE);
...
//		description.setText(mDescription);
	...
	public void responseImageFromBackground(Bitmap response) {
		...
		buttonSetWallpaper.setVisibility(Button.VISIBLE);
		image.setImageBitmap(response);
		title.setText( list.get(CURRENT_FEED_NUMBER).getTitle() );
		date.setText( list.get(CURRENT_FEED_NUMBER).getDate() );	
		description.setText( list.get(CURRENT_FEED_NUMBER).getDescription() );
		
		backgroundImage = response;
		if (CURRENT_FEED_NUMBER == 0) {
			left.setVisibility(ImageButton.INVISIBLE);
		}else {
			left.setVisibility(ImageButton.VISIBLE);
		}

		if (CURRENT_FEED_NUMBER == list.size()-1) {
